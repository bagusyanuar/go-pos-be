...
// Delete implements ProductCategoryHandler.
func (p *productCategoryHandlerImpl) Delete(ctx *fiber.Ctx) error {
	id := ctx.Params("id")

	err := p.ProductCategoryService.Delete(ctx.UserContext(), id)
	if err != nil {
...
// Update implements ProductCategoryHandler.
func (p *productCategoryHandlerImpl) Update(ctx *fiber.Ctx) error {
	id := ctx.Params("id")
	...
	err = p.ProductCategoryService.Update(ctx.UserContext(), id, req)
	if err != nil {
		return ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"code":    fiber.StatusInternalServerError,
			"message": err.Error(),
